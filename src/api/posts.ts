...
async function newPost(
    ...
    try {
        await queryDatabase(
            'INSERT INTO topics(userid, topic, description, tags) VALUES ($1,$2,$3,$4);',
            [userId, topic, description, tags]
        )
    } catch (error) {
        console.error(error)
    }
...
async function getPostsVotes(postId: number, vote: boolean) {
    const result = await queryDatabase('SELECT * FROM topics WHERE id = $1', [
        postId,
    ])
    if (vote) {
...
async function getUserPost(userId: number | string) {
    return await queryDatabase('SELECT * FROM topics WHERE userid = $1', [
        userId,
    ])
}
...
async function getPostId(userId: number, topic: string, description: string) {
    return await queryDatabase(
        'SELECT * FROM topics WHERE userid = $1 AND topic = $2 AND description = $3',
